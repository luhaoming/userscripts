...
1. **⚠️ Tampermonkey MV3 權限設定 (最常見原因)**
如果您使用的是新版瀏覽器或 Tampermonkey v5.0+ (MV3 版本)，請務必檢查權限：
* 前往瀏覽器的「擴充功能管理」頁面 (Extension Settings)。
* 找到 Tampermonkey。
* 確認 **「開發人員模式」** 或 **「允許使用者指令碼」 (Allow User Scripts)** 的開關已開啟（呈現藍色）。
* *這是新版瀏覽器的安全性限制，必須手動開啟腳本才能運作。*


2. **檢查腳本管理器**：確認 Tampermonkey 是否已啟用。
3. **檢查網址**：當前網站是否在支援列表中 (ChatGPT/Claude/Gemini/Grok)。
4. **重新整理**：嘗試按下 F5 重新整理頁面。
